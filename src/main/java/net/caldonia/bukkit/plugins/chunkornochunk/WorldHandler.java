...
public class WorldHandler {
    ...
    public void worldLoad(World world) {
        ...
        if (!keepSpawnInMemory) {
            ...
            for (Chunk chunk : world.getLoadedChunks()) {
                ...
                if (unload) {
                    plugin.getLogger().fine("World '" + worldName + "' unloading chunk " + chunk.getX() + "," + chunk.getZ() + ".");
    ...
    public void chunkLoad(Chunk chunk, boolean isNew) {
        ...
        if (isNew && preventNewChunks) {
            plugin.getLogger().fine("World '" + worldName + "' preventing new chunk " + chunk.getX() + "," + chunk.getZ() + ".");
    ...
    public boolean chunkUnload(Chunk chunk) {
        ...
        for (WorldArea worldArea : worldAreas.values()) {
            if (worldArea.includesChunk(chunk)) {
                plugin.getLogger().fine("World '" + worldName + "' preventing chunk unload due to area '" + worldArea.getName() + "' for " + chunk.getX() + "," + chunk.getZ() + ".");
                // Cancel the unload.
                return true;
            }
        }
        ...
        if (!safeUnloadingChunks.contains(point) && preventChunkSaves) {
            plugin.getLogger().fine("World '" + worldName + "' canceling unload to prevent save for " + chunk.getX() + "," + chunk.getZ() + ".");
        ...
        plugin.getLogger().fine("World '" + worldName + "' allowing unload for " + chunk.getX() + "," + chunk.getZ() + ".");
